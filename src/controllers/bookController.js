...
const createBook = async (req, res) => {
  try {
    ...
    if (!dataValidation(reqBody))
      return res
        .status(400)
        .send({ status: false, message: "Please fill the data" });

    if (Object.keys(reqBody).length > 8)
      return res
        .status(400)
        .send({ status: false, message: "You can't add extra field" });
    //------------------------------title validation-----------------------------------
    if (!title)
      return res
        .status(400)
        .send({ status: false, message: "title isn't present" });

    if (!isValidText(title))
      return res
        .status(400)
        .send({ status: false, message: "title isn't valid" });
    //------------------------------excerpt validation-----------------------------------
    if (!isValidText(excerpt))
      return res
        .status(400)
        .send({ status: false, message: "excerpt isn't present" });

    if (!excerpt)
      return res
        .status(400)
        .send({ status: false, message: "excerpt isn't valid" });

    //---------------------------------userId validation------------------------------
    if (!userId)
      return res
        .status(400)
        .send({ status: false, message: "userId isn't present" });

    if (!isValidObjectId(userId))
      return res
        .status(400)
        .send({ status: false, message: "userId isn't valid" });

    //---------------------------------ISBN validation------------------------------
    if (!ISBN)
      return res
        .status(400)
        .send({ status: false, message: "ISBN isn't present" });

    if (!isValidIsbn(ISBN))
      return res
        .status(400)
        .send({ status: false, message: "ISBN isn't valid" });

    //---------------------------------category validation------------------------------
    if (!category)
      return res
        .status(400)
        .send({ status: false, message: "category isn't present" });

    if (!isValidName(category))
      return res
        .status(400)
        .send({ status: false, message: "category isn't valid" });

    //---------------------------------category validation------------------------------
    if (!subcategory)
      return res
        .status(400)
        .send({ status: false, message: "subcategory isn't present" });

    if (!isValidName(subcategory))
      return res
        .status(400)
        .send({ status: false, message: "subcategory isn't valid" });

    //--------------------------------- finding user------------------------------
    if (!isValidDate(releasedAt))
      return res.status(400).send({
        status: false,
        message: "Please use 'YYYY-MM-DD' this format",
      });
    ...
    for (let i = 0; i < existBook.length; i++) {
      if (existBook[i].title === title)
        return res
          .status(400)
          .send({ status: false, msg: "title is Duplicate" });
    }
    ...
    for (let i = 0; i < existBook.length; i++) {
      if (existBook[i].ISBN === ISBN)
        return res
          .status(400)
          .send({ status: false, msg: "ISBN is Duplicate" });
    }
...
let getBooks = async (req, res) => {
  ...
  let result = await bookModel
    .find({ $or: [{ userId: userId }, { category: category }] })
    .select({ createdAt: 0, updatedAt: 0, __v: 0, subcategory: 0, ISBN: 0 })
    .sort({ title: 1 });
...
const getBookById = async (req, res) => {
  ...
  try {
    ...
    if (!isValidObjectId(bookID))
      return res
        .status(400)
        .send({ status: false, message: `This bookId ${bookID} is Invalid` });
    ...
    if (bookId.isDeleted == true)
      return res.status(404).send({
        status: false,
        message: `The Book Title '${bookId.title}' has been Deleted`,
      });

    let findBook = await bookModel.findById(bookID).select({ __v: 0, ISBN: 0 });
    let review = await reviewModel.find({ bookId: bookID }).select({
      isDelete: 0,
      createdAt: 0,
      updatedAt: 0,
      isDeleted: 0,
